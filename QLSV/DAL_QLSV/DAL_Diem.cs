...
namespace DAL_QLSV
{
    public class DAL_Diem : DBConnect
    {
        ...
        public DataTable getTDiemByIdMH(string id_MH)
        {
            string SQL = string.Format("SELECT * FROM dbo.Diem WHERE id_MH = '{0}'", id_MH);
            SqlDataAdapter diem = new SqlDataAdapter(SQL, conn);
            DataTable dtDiem = new DataTable();
            diem.Fill(dtDiem);
            return dtDiem;
        ...
        public DataTable getTMHByIdGV(string id_GV)
        {
